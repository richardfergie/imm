...
instance Default Config where
    def = Config {
        _maildir       = "feeds",
        _dateParsers   = [
            return . zonedTimeToUTC <=< parseTimeRFC2822,
            return . zonedTimeToUTC <=< parseTimeRFC3339,
            T.parseTime T.defaultTimeLocale "%a, %d %b %G %T",
            T.parseTime T.defaultTimeLocale "%Y-%m-%d",
            T.parseTime T.defaultTimeLocale "%e %b %Y",
            T.parseTime T.defaultTimeLocale "%a, %e %b %Y %k:%M:%S %z",
            T.parseTime T.defaultTimeLocale "%a, %e %b %Y %T %Z"],
